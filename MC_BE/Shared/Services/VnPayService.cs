...
public class VnPayService : IVnPayService
{
        ...
        string ipAddress)
    {
        ...
        var data = new Dictionary<string, string>
        {
            ...
            ["vnp_Locale"] = "vn",
            ["vnp_OrderInfo"] = payment.OrderInfo ?? "MSEEK Course Payment",
            ["vnp_OrderType"] = "other",
            ["vnp_ReturnUrl"] = _settings.ReturnUrl,
            ["vnp_TxnRef"] = payment.MerchantTxnRef,
        ...
        return $"{_settings.PaymentUrl}?{hashData}&vnp_SecureHash={secureHash}";
    }
    ...
    public bool ValidateSignature(IQueryCollection query)
    {
        ...
        var hashData = BuildQueryString(data);
        var expectedHash = HmacSha512(_settings.HashSecret, hashData);

        return string.Equals(receivedHash, expectedHash, StringComparison.OrdinalIgnoreCase);
    }

    public async Task<VnPayQueryResultDto> QueryTransactionAsync(Payment payment, string ipAddress)
    {
        ValidateConfiguration();

        var vnNow = GetVietnamTime();
        var transactionDate = ConvertToVietnamTime(payment.CreatedAt);
        var requestId = Guid.NewGuid().ToString("N")[..24];
        var createDate = vnNow.ToString("yyyyMMddHHmmss");
        var transactionDateString = transactionDate.ToString("yyyyMMddHHmmss");
        var orderInfo = $"Query payment {payment.MerchantTxnRef}";
        ...
        var body = new Dictionary<string, string>
        {
            ...
            ["vnp_TransactionDate"] = transactionDateString,
            ["vnp_CreateDate"] = createDate,
            ["vnp_IpAddr"] = NormalizeIp(ipAddress),
            ["vnp_SecureHash"] = HmacSha512(_settings.HashSecret, checksumData)
        };

        var response = await _httpClient.PostAsJsonAsync(_settings.ApiUrl, body);
        ...
        if (!response.IsSuccessStatusCode)
        {
            return new VnPayQueryResultDto
            {
                ...
                Message = "Unable to retrieve payment information from VNPay.",
                RawResponse = raw
            };
        }
        ...
        string? GetValue(string key) =>
            root.TryGetProperty(key, out var prop) ? prop.ToString() : null;

        decimal? amount = null;
        if (decimal.TryParse(GetValue("vnp_Amount"), out var rawAmount))
        {
        ...
        return new VnPayQueryResultDto
        {
            RequestSucceeded = GetValue("vnp_ResponseCode") == "00",
            ResponseCode = GetValue("vnp_ResponseCode"),
            Message = GetValue("vnp_Message"),
            TransactionStatus = GetValue("vnp_TransactionStatus"),
            TransactionNo = GetValue("vnp_TransactionNo"),
            BankCode = GetValue("vnp_BankCode"),
            Amount = amount,
            RawResponse = raw
        };
    }
    ...
    private static DateTime GetVietnamTime() => ConvertToVietnamTime(DateTime.UtcNow);
