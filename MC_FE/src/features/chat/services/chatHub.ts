...
const BACKEND_URL = (
  import.meta.env.VITE_BACKEND_URL || "http://localhost:5239"
).replace(/\/+$/, "");
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (hub.state === HubConnectionState.Disconnected) {
        window.clearInterval(timer);
        reject(new Error("ChatHub đã ngắt kết nối."));
        return;
      }

      if (Date.now() - startedAt >= timeoutMs) {
        window.clearInterval(timer);
        reject(new Error("Không thể kết nối ChatHub."));
      }
    }, 100);
  });
}
...
export async function getOnlineUsers(): Promise<number[]> {
  ...
  const result = await hub.invoke<number[]>("GetOnlineUsers");

  return Array.isArray(result)
    ? result.map(Number).filter(Number.isFinite)
    : [];
}
